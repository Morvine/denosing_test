...
class Decoder(nn.Module):
    ...
    def __init__(self, Ns, Ss, depth, num_tfc):
        ...
        for i in range(self.depth):
            self.dblocks.append(
                D_Block(layer_idx=i, N0=self.Ns[i + 1], N=self.Ns[i], S=self.Ss[i], num_tfc=num_tfc))
...
class Encoder(nn.Module):
    ...
    def __init__(self, N0, Ns, Ss, depth, num_tfc):
        ...
        for i in range(self.depth):
            ...
            self.eblocks.append(
                E_Block(layer_idx=i, N0=Nin, N01=self.Ns[i], N=self.Ns[i + 1], S=self.Ss[i], num_tfc= self.num_tfc))

        self.i_block = I_Block(self.Ns[self.depth], self.Ns[self.depth],  self.num_tfc)
...
class MultiStage_denoise(nn.Module):
    ...
    def __init__(self, depth=6, use_fencoding=False, f_dim=None, use_SAM=True, num_stages=2, num_tfc=3):
        super(MultiStage_denoise, self).__init__()
        self.depth = depth
        self.num_tfc = num_tfc
        Nin = 1
        if use_fencoding:
            self.freq_encoding = AddFreqEncoding(f_dim)
            Nin = 12  # hardcoded
        self.use_sam = use_SAM
        self.use_fencoding = use_fencoding
        ...
        self.encoder_s1 = Encoder(self.Ns[0], self.Ns, self.Ss, self.depth, self.num_tfc)
        self.decoder_s1 = Decoder(self.Ns, self.Ss, self.depth, self.num_tfc)
        ...
        if self.num_stages > 1:
            ...
            self.encoder_s2 = Encoder(2 * self.Ns[0], self.Ns, self.Ss, self.depth, self.num_tfc)
            self.decoder_s2 = Decoder(self.Ns, self.Ss, self.depth, self.num_tfc)
